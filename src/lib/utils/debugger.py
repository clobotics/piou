...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif dataset == 'dota' or dataset == 'retail50k' or dataset == 'BottleTracking':
  ...
  #   rect = [box[0], box[1], box[2], box[3], box[4], box[5], box[6], box[7]]
  ...
  def add_rotation_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default'):
    bbox = np.array(bbox, dtype=np.int32)
    rect = self._rotation_bbox_to_segmentation(bbox)
    # cat = (int(cat) + 1) % 80
    cat = int(cat)
    # print('cat', cat, self.names[cat])
    c = self.colors[cat][0][0].tolist()
    ...
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    corners = rect.reshape(-1, 1, 2).astype(int)
    cv2.polylines(self.imgs[img_id], [corners], True, c, 2)

    if show_txt:
      cv2.rectangle(self.imgs[img_id],
                    (int(rect[0]), int(rect[1]) - cat_size[1] - 2),
                    (int(rect[0]) + cat_size[0], bbox[1] - 2), c, -1)
      cv2.putText(self.imgs[img_id], txt, (int(rect[0]), int(rect[1]) - 2),
                  font, 0.5, (0, 0, 0), thickness=1, lineType=cv2.LINE_AA)
